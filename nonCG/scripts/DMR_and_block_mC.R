...
mC <- mclapply(list_of_BSseq, function(BSseq, list_of_regions, fac) {
  ...
  lapply(list_of_regions, function(regions) {
    ...
    ave_mC <- t(apply(outMatrix, 1, function(xx) tapply(xx, fac, mean)))
    rownames(ave_mC) <- names(regions)
    list(sample_level = outMatrix,
         condition_level = ave_mC)
  })
}, list_of_regions = DMRs_and_blocks, fac = "Tissue")

list_of_SEs <- lapply(names(list_of_regions), function(n) {
  ...
  ave_mC_condition_level <- lapply(ave_mC_condition_level, function(x) {
    colnames(x)[colnames(x) == "NA"] <- "NAcc"
    x
  })
...
# TODO: Check rownames/colnames/names worked properly
