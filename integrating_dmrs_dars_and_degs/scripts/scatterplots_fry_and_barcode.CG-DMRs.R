# Correlation of CG-DMRs with RNA-seq profiles (adapted from
...
ol <- findOverlaps(dmrs_NAvsBA9pos,
                   reduce(c(unlist(promoters_by_gene[lfc$gene]),
                          ...
                          ignore.strand = TRUE))
meanDiffs <- tapply(
  X = (dmrs_NAvsBA9pos$`NA` - dmrs_NAvsBA9pos$BA9)[queryHits(ol)],
...
lfc <- rna_atac_meth %>%
  dplyr::select(gene, gene_symbol, DE, expLogFC)
lfc$meanDiff <- NA_real_
ol <- findOverlaps(dmrs_NAvsBA9pos, promoters_by_gene[lfc$gene],
                   ignore.strand = TRUE)
...
pdf("../figures/logFC_vs_CG-DMRs.promoters.pdf",
    height = 7,
    width = 7)
...
pdf("../figures/barcodeplot.CG-DMRs.promoters.pdf",
    height = 7,
    width = 7)
gw <- lfc$meanDiff
gw[is.na(gw)] <- 0
barcodeplot(lfc$expLogFC,
            gene.weights = gw,
            weights.label = expression(paste(Delta, "mCG", sep = "")),
...
lfc <- rna_atac_meth %>%
  dplyr::select(gene, gene_symbol, DE, expLogFC)
lfc$meanDiff <- NA_real_
ol <- findOverlaps(dmrs_NAvsBA9pos, gencode_features$genes[lfc$gene],
                   ignore.strand = TRUE)
# NOTE: If multiple hits, take the one with the biggest meanDiff
meanDiffs <- tapply(
  X = (dmrs_NAvsBA9pos$`NA` - dmrs_NAvsBA9pos$BA9)[queryHits(ol)],
...
pdf("../figures/logFC_vs_CG-DMRs.gene_body.pdf",
    height = 7,
    width = 7)
plot(expLogFC ~ meanDiff,
     data = lfc,
...
pdf("../figures/barcodeplot.CG-DMRs.gene_body.pdf",
    height = 7,
    width = 7)
gw <- lfc$meanDiff
gw[is.na(gw)] <- 0
...
lfc <- rna_atac_meth %>%
  dplyr::select(gene, gene_symbol, DE, expLogFC)
lfc$meanDiff <- NA_real_
ol <- findOverlaps(dmrs_NAvsBA9pos, gene_excluding_promoters[lfc$gene],
                   ignore.strand = TRUE)
# NOTE: If multiple hits, take the one with the biggest meanDiff
meanDiffs <- tapply(
  X = (dmrs_NAvsBA9pos$`NA` - dmrs_NAvsBA9pos$BA9)[queryHits(ol)],
...
pdf("../figures/logFC_vs_CG-DMRs.gene_body_excluding_promoters.pdf",
    height = 7,
    width = 7)
plot(expLogFC ~ meanDiff,
     data = lfc,
...
pdf("../figures/barcodeplot.CG-DMRs.gene_body_excluding_promoter.pdf",
    height = 7,
    width = 7)
gw <- lfc$meanDiff
gw[is.na(gw)] <- 0
...
pdf("../figures/logFC_vs_CG-DMRs.FANTOM5_enhancers.pdf",
    height = 7,
    width = 7)
plot(expLogFC ~ meanDiff,
     data = lfc,
...
pdf("../figures/barcodeplot.CG-DMRs.FANTOM5_enhancers.pdf",
    height = 7,
    width = 7)
gw <- lfc$meanDiff
gw[is.na(gw)] <- 0
