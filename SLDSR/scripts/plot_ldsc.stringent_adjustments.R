# Plot LDSC results for
# 'adjusting for baseline + CNS + chromHMM_union + H3K27ac' analyses
# Peter Hickey
# 2018-08-07
...
fls <- unlist(lapply(names(brain_categories), function(bc) {
  list.files("../output/ldsc",
             pattern = glob2rx(
               paste0(bc, ".adjusting_for_baseline_and_CNS_and_chromHMM_union_and_H3K27ac.*Phase1.results")),
             full.names = TRUE)
}))
...
x <- bind_rows(lapply(fls, function(fl) {
  suppressMessages(read_tsv(fl)) %>%
    filter(Category %in% c("L2_4",
                           "Conserved_LindbladToh_0",
                           "DHS_peaks_Trynka_0", "Enhancer_Andersson_0",
                           "CNS_1", "L2_2", "L2_3")) %>%
...
x_stratified %>%
  ...
  rename(Feature = `Pretty Category`,
         ...
         Stratum = `strata`) %>%
  write_csv("../tables/LDSC_results.stringent_adjustments.csv")
...
ggsave("../figures/Coefficient_Z-score.stringent_adjustments.pdf",
       g,
       height = 6,
       width = 7)
g <- x_stratified %>%
  ...
  guides(col = FALSE, shape = FALSE, size = FALSE) +
  ylim(ylim_coefficient_score)
ggsave("../figures/Coefficient_Z-score.stringent_adjustments.stratified.pdf",
       g,
       height = 4,
       width = 5)
...
ggsave("../figures/Enrichment.stringent_adjustments.pdf",
       g,
       height = 6,
       width = 7)
...
ggsave("../figures/Enrichment.stringent_adjustments.sig_stratified.pdf",
       g,
       height = 4,
       width = 5)
